...
let key = '1WTLyrAp91dJP2tmKFBEuJRhPkazG-uqw8Ab63fXNvAY';
  ...
  for (let i = 0; i < data.length; i++) {
    bubbles.push(
      new Bubble(
        ...
        data[i]["Is Daylight Savings Time good or bad?"],
        data[i]["An event that should become an Olympic sport"],
        data[i]["What's the deal with airline food?"])); // THESE NEED TO MATCH SPREADSHEET
...
// my Bubble class
class Bubble {
  constructor(name, daylightsavings, olympic, airlinefood) {
    // only the order of these parameters matters!
    this.name = name;
    this.daylightsavings = daylightsavings;
    this.olympic = olympic;
    this.airlinefood = airlinefood;
